...
<summary>Click to view Prompt</summary>
    ...
    ```markdown
    I have attached the generated script and the .ttl which it generated. Evaluate it against the existing 20 competency questions and give another prompt to generate 10 additional validation competency questions for real-world application and corrections if required.

    ```
    ontology_builder_initial.py (Script provided by builder chat)
    tbox_ontology.ttl file from that script. (file produced by ontology_builder)
    ```
    ```
...
<summary>Click to view Prompt</summary>
    ...
    3. Generate 10 additional validation competency questions for real-world application using the existing ontology and CQ structure, and extend the ontology only where necessary to support them.
    4. Remove or simplify any classes or properties that are not necessary for answering the first 20 competency questions and the 10 additional validation competency questions.
...
<summary>Click to view Prompt</summary>
    ...
    tbox_ontology.ttl (produced by updated script)
    ```
...
## Initiating the Task
Goal: Implement a RAG strategy to resolve some of the incompleteness found

<details>
<summary>Click to view Prompt</summary>

    ```markdown
    “Here is a pdf explaining my project task, I need to implement the RAG strategy task. I’d like to implement RAG for the following two gaps: Accessibility assessments (TfL API + LLM formatting) and Missing fare zones (TfL API + LLM disambiguation). I have attached 3 Turtle (.ttl) files below. Please help me generate a python script that uses RAG to resolve these two gaps in our ontology.”
    ```

</details>

## Fare Zone Disambiguation


Goal: Given retrieved TfL API zone data as context, return the single best fare zone integer for a station (AUGMENT step).

<details>
<summary>Click to view Prompt</summary>

    ```markdown
    "You are an expert on the London Underground fare zone system.

    ## Retrieved context from the TfL API (use this as your primary source): {tfl_context}
...
Goal: Given retrieved TfL accessibility data as context, produce a structured JSON object representing the station's wheelchair accessibility assessment (AUGMENT step).

<details>
<summary>Click to view Prompt</summary>

    ```markdown
    You are a knowledge graph engineer working on the London Underground ontology.
...
Goal: Extact information and data from an LLM
